...
bool is_il_program_supported(cl_context context)
{
    ...
    cl_int error = tdispatch->clGetContextInfo(context, CL_CONTEXT_NUM_DEVICES, sizeof(num_devices),
                                               &num_devices, nullptr);
    if (error != CL_SUCCESS) return false;
    std::vector<cl_device_id> devices(num_devices);
    error = tdispatch->clGetContextInfo(
        context, CL_CONTEXT_DEVICES, num_devices * sizeof(cl_device_id), devices.data(), nullptr);
    if (error != CL_SUCCESS) return false;
    for (const auto& device : devices)
    {
        std::size_t il_version_length{};
        error = tdispatch->clGetDeviceInfo(device, CL_DEVICE_IL_VERSION, 0, nullptr,
                                           &il_version_length);
...
CL_API_ENTRY cl_program CL_API_CALL clCreateProgramWithBinary_wrap(cl_context context,
                                                                   ...
                                                                   cl_int* errcode_ret)
{
    auto program = tdispatch->clCreateProgramWithBinary(context, num_devices, device_list, lengths,
...
clCreateProgramWithBuiltInKernels_wrap(cl_context context,
                                       ...
                                       cl_int* errcode_ret)
{
    auto program = tdispatch->clCreateProgramWithBuiltInKernels(context, num_devices, device_list,
...
bool are_global_variables_supported(cl_context context)
{
    ...
    for (const auto& device : devices)
    {
        ...
        error =
            tdispatch->clGetDeviceInfo(device, CL_DEVICE_MAX_GLOBAL_VARIABLE_SIZE,
                                 ...
                                 void* user_data)
{
    ...
    {
        const cl_int error =
            tdispatch->clSetProgramReleaseCallback(entry_it->second.program, pfn_notify, user_data);
        if (error != CL_SUCCESS) return error;
    }
                                                                        ...
                                                                        const void* spec_value)
{
    ...
    if (!is_il_program_supported(entry_it->second.context)) return CL_INVALID_OPERATION;
                    ...
                    void* user_data)
{
    ...
    std::visit(
        utils::overloads{
            [&](binary_program) {
                error = tdispatch->clBuildProgram(entry_it->second.program, num_devices,
            ...
            [&](const auto& source) {
                try
                {
                    ...
                    {
                        new_program = program_cache->fetch_or_build_il(source, context, devices,
                                                                       entry_it->second.options);
                    }
                      ...
                      void* user_data)
{
    ...
    return tdispatch->clCompileProgram(entry_it->second.program, num_devices, device_list, options,
                                       num_input_headers, wrapped_input_headers.data(),
                   ...
                   cl_int* errcode_ret)
{
    ...
    for (size_t program_idx = 0; program_idx < num_input_programs; ++program_idx)
    {
        ...
        auto entry_it = program_entries.find(index);
        if (entry_it == program_entries.end() || entry_it->second.program == nullptr)
        {
    ...
    const cl_program linked_program =
        tdispatch->clLinkProgram(context, num_devices, device_list, options, num_input_programs,
                                                      ...
                                                      size_t* param_value_size_ret)
{
    ...
    switch (param_name)
    {
        case CL_PROGRAM_REFERENCE_COUNT:
            ...
            if (param_value) *static_cast<cl_uint*>(param_value) = entry_it->second.reference_count;
        ...
        case CL_PROGRAM_CONTEXT:
            ...
            if (param_value) *static_cast<cl_context*>(param_value) = entry_it->second.context;
            if (param_value_size_ret) *param_value_size_ret = sizeof(cl_context);
            return CL_SUCCESS;
        case CL_PROGRAM_NUM_DEVICES:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_NUM_DEVICES,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
            return tdispatch->clGetContextInfo(entry_it->second.context, CL_CONTEXT_NUM_DEVICES,
                                               param_value_size, param_value, param_value_size_ret);
        case CL_PROGRAM_DEVICES:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_DEVICES,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
            return tdispatch->clGetContextInfo(entry_it->second.context, CL_CONTEXT_DEVICES,
                                               param_value_size, param_value, param_value_size_ret);
        case CL_PROGRAM_SOURCE:
            if (std::holds_alternative<std::string>(entry_it->second.source))
            {
                const auto& source = std::get<std::string>(entry_it->second.source);
            ...
            return CL_SUCCESS;
        case CL_PROGRAM_IL:
            if (std::holds_alternative<std::vector<char>>(entry_it->second.source))
            {
                const auto& il = std::get<std::vector<char>>(entry_it->second.source);
        ...
        case CL_PROGRAM_BINARY_SIZES:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_BINARY_SIZES,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
            else if (param_value_size_ret)
        ...
        case CL_PROGRAM_BINARIES:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_BINARIES,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
            else if (param_value_size_ret)
        ...
        case CL_PROGRAM_NUM_KERNELS:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_NUM_KERNELS,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
        ...
        case CL_PROGRAM_KERNEL_NAMES:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramInfo(unwrapped_program, CL_PROGRAM_KERNEL_NAMES,
                                                   param_value_size, param_value,
                                                   param_value_size_ret);
            }
            if (param_value && param_value_size < 1) return CL_INVALID_VALUE;
                                                           ...
                                                           size_t* param_value_size_ret)
{
    ...
    switch (param_name)
    {
        case CL_PROGRAM_BUILD_STATUS:
            if (auto unwrapped_program = entry_it->second.program; unwrapped_program)
            {
                return tdispatch->clGetProgramBuildInfo(unwrapped_program, device,
            ...
            if (param_value)
                *static_cast<cl_build_status*>(param_value) =
                    entry_it->second.build_attempted ? CL_BUILD_ERROR : CL_BUILD_NONE;
                                                       ...
                                                       cl_int* errcode_ret)
{
    ...
    return tdispatch->clCreateKernel(entry_it->second.program, kernel_name, errcode_ret);
}
                                                              ...
                                                              cl_uint* num_kernels_ret)
{
    ...
    return tdispatch->clCreateKernelsInProgram(entry_it->second.program, num_kernels, kernels,
                                               num_kernels_ret);
}
    ...
    switch (param_name)
    {
        ...
        case CL_KERNEL_ATTRIBUTES: {
            ...
            if (std::holds_alternative<std::string>(found_it->second.source))
            {
                ...
                error = tdispatch->clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, 0, nullptr,
                                                   &kernel_name_size);
                if (error != CL_SUCCESS) return error;
                std::string kernel_name;
                kernel_name.resize(kernel_name_size);
                error = tdispatch->clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME,
                                                   kernel_name_size, kernel_name.data(), nullptr);
                if (error != CL_SUCCESS) return error;
                const auto& source = std::get<std::string>(found_it->second.source);
                ...
                const auto tmp_program = tdispatch->clCreateProgramWithSource(
                    found_it->second.context, 1, &source_str, &source_size, &error);
                if (error != CL_SUCCESS) return error;
                error = tdispatch->clBuildProgram(
                    tmp_program, 0, nullptr, found_it->second.options.c_str(), nullptr, nullptr);
                if (error != CL_SUCCESS)
                {
                ...
                const auto tmp_kernel =
                    tdispatch->clCreateKernel(tmp_program, kernel_name.c_str(), &error);
                if (error != CL_SUCCESS)
                {
                    tdispatch->clReleaseProgram(tmp_program);
                    return error;
                ...
                error =
                    tdispatch->clGetKernelInfo(tmp_kernel, CL_KERNEL_ATTRIBUTES, param_value_size,
                                               param_value, param_value_size_ret);
        ...
        default:
            return tdispatch->clGetKernelInfo(kernel, param_name, param_value_size, param_value,
                                              param_value_size_ret);
    }
}
