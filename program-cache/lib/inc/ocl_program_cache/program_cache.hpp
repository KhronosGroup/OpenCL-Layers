...
/// @brief Error thrown when a cl::Program is passed which should have been built
/// previously.
...
public:
    ...
    /// @param context OpenCL context to build the programs for. If \c nullptr is passed,
    /// then the default context is used.
    /// @param cache_root Path to the program cache root on the filesystem. If \c nullopt
    /// is passed, the platform dependent default location is used.
    program_cache(
        std::shared_ptr<const cl::Context> context = nullptr,
        const std::optional<std::filesystem::path>& cache_root = std::nullopt);

    /// @brief Loads cached binaries for all devices associated with the \c cl::Context
    /// passed in the constructor and builds a \c cl::Program.
    /// @param key Key to the cache entries. The key must be equal to the key passed to a
    /// previous \c store call to retrieve the same binaries.
    /// @return The built \c cl::Program if a cache entry was found for all devices,
    /// \c std::nullopt otherwise.
    std::optional<cl::Program> fetch(std::string_view key) const;

    /// @brief Loads cached binaries for all devices passed and returns a built \c cl::Program.
    /// @param key Key to the cache entries. The key must be equal to the key passed to a
    /// previous \c store call to retrieve the same binaries.
    ...
    /// @brief Stores the binary representation of a \c cl::Program in the cache.
    /// @param program The program to store. It must be built previously, otherwise
    /// \c unbuilt_program_error is thrown.
    /// @param key The key to the cache entry, which can be used to retrieve the cache
    /// entry later via \c fetch.
    void store(const cl::Program& program, std::string_view key) const;

    /// @brief Builds OpenCL source code to a \c cl::Program and stores it in the cache.
    /// If the program existed in the cache previously, loads it back from the cache
    /// without building.
    ...
    /// @note The cache lookup considers the passed source code, the passed options,
    /// the platform's version and the devices to which the code is compiled.
    /// In this overload, the code is compiled to all devices associated with the
    /// \c cl::Context passed in the constructor.
    ...
    /// @brief Builds OpenCL IL code to a \c cl::Program and stores it in the cache.
    /// If the program existed in the cache previously, loads it back from the cache
    /// without building.
